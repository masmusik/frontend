...
import {ReportMediaItemModalComponent} from './report-media-item-modal/report-media-item-modal.component';
...
export abstract class ContextMenuComponent<T> {
    ...
    public openReportModal() {
        ...
        const data = {mediaItem: this.data.item, type: this.data.type};
        this.modal.open(ReportMediaItemModalComponent, data).afterClosed().subscribe(shared => {
            if ( ! shared) return;
            //send emails
        })
    }
